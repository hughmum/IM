package com.mu.im.service.group.service.impl;
...
import com.mu.im.common.enums.GroupTypeEnum;
...
public class ImGroupMemberServiceImpl implements ImGroupMemberService {
    ...
    /**
     * 添加群成员，内部调用
     * 首先通过 imUserService.getSingleUserInfo()检查成员ID是否存在。如果成员不存在,则返回错误响应。
     *
     * 然后检查传入的角色是否为OWNER。如果是,则检查该群是否已经有OWNER。如果有,则返回错误。这是为了确保每个群只有一个OWNER。
     *
     * 然后检查该成员是否已经在该群中,使用 ImGroupMemberEntity。
     *
     * 如果该成员不存在(即首次添加),则插入新的记录。
     *
     * 如果该成员的角色为LEAVE(之前离开),则使用新角色更新该记录。
     *
     * 根据插入/更新的结果返回成功或错误响应。
     * @param groupId
     * @param appId
